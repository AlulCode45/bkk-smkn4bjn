...
@section('content')
    ...
    <div class="container">
        <div class="bg-light p-3 mb-5">
            <form action="{{ route('data_lowongan') }}" method="get">
                <div class="search">
                    <input type="text" class="searchTerm" placeholder="Tulis Perusahaan atau Lowongan..."
        ...
        <div class="row row-cols-1 row-cols-md-2">
            @foreach ($lowongan as $e)
                    ...
                    <div class="solution_cards_box">
                        <div class="solution_card">
                            ...
                            <div class="d-flex">
                                ...
                                <div class="solu_title ms-3 mt-2">
                                    <h2 class="title-lowongan">{{ $e->nama }}</h2>
                                </div>
                            </div>
                            ...
                            <div class="solu_description">
                                <p>
                                    ...
                                    {{ $e->created_at ? date('D, d M Y H:i', strtotime($e->created_at . '+ 7 hours')) : 'Tidak ada Waktu' }}
                                </p>
                                <p></p>
                                {{ $e->judul }}
                                </p>
                                <div class="">
                                    @foreach ($e->jurusan as $data)
                                        <span class="badge rounded-pill bg-light text-dark">{{ $data->nama_jurusan }}</span>
                                    @endforeach
                                </div>
                                <a href="{{ route('siswa.viewLowongan', $e->id_lowongan) }}">
                                    <button type="button"class="read_more_btn">
                                        <ion-icon name="checkmark-outline" class="me-1" size="medium"></ion-icon>Lamar
                                    </button>
                                </a>
                                <button type="button" data-bs-toggle="modal"
                                    data-bs-target="#Label{{ md5($loop->iteration) }}" class="read_more_btn mt-3">
                                    <ion-icon name="list-circle-outline" class="me-1"></ion-icon>Detail Lowongan
                                </button>
                ...
                <div class="modal fade" id="Label{{ md5($loop->iteration) }}" tabindex="-1"
                    aria-labelledby="Label{{ md5($loop->iteration) }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="Label{{ md5($loop->iteration) }}">{{ $e->nama }}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="flex-column">
                                    <span class="badge bg-light text-dark">Lowongan</span>
                                    <h6 class="ms-3 mt-2" style="opacity: 80%">{{ $e->judul }}</h6>
                                </div>
                                <div class="flex-column">
                                    <span class="badge bg-light text-dark">Gaji</span>
                                    <h6 class="ms-3 mt-2" style="opacity: 80%">{{ $e->gaji }}</h6>
                                </div>
                                <div class="flex-column">
                                    <span class="badge bg-light text-dark">Lokasi</span>
                                    <h6 class="ms-3 mt-2" style="opacity: 80%">{{ $e->lokasi }}</h6>
                                </div>
                                <div class="flex-column">
                                    <span class="badge bg-light text-dark">Penempatan</span>
                                    <h6 class="ms-3 mt-2" style="opacity: 80%">{{ $e->penempatan }}</h6>
                                </div>
                                <div class="card mt-3">
                                ...
                                <div>
                                    <hr>
                                    @foreach ($e->jurusan as $data)
                                        <span class="badge rounded-pill bg-primary">{{ $data->nama_jurusan }}</span>
                                    @endforeach
                                </div>
                            </div>
