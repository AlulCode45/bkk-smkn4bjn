...
class PerusahaanController extends Controller
{
    ...
    {
        $data = [
            'lowongan' => LowonganModel::join('data_perusahaan', 'data_perusahaan.id', '=', 'lowongan.id_perusahaan')
                ->join('users', 'users.id', '=', 'data_perusahaan.id_user', 'inner')
                ->where('lowongan.id_pembuat', Auth::user()->id)
                ->orWhere('lowongan.id_pembuat', 1)
    ...
    {
        $data = [
            'testimoni' => TestimoniModel::where('id_user', Auth::user()->id)->first()
        ];
        return view('Perusahaan.testimoni', $data);
    }
    public function updateTestimoni(Request $request)
    {
        ...
        if (TestimoniModel::find($request->id) == null) {
            $save = [
                'id_user' => Auth::user()->id,
                'testimoni' => $request->testimoni
            ];
            if (TestimoniModel::insert($save)) {
                return redirect()->back()->with('success', 'Berhasil menambah testimoni');
            } else {
                return redirect()->back()->with('error', 'Gagal menambah testimoni');
            }
        } else {
            $update = [
                'testimoni' => $request->testimoni
            ...
            if (TestimoniModel::find($request->id)->update($update)) {
                return redirect()->back()->with('success', 'Berhasil mengedit testimoni');
            } else {
                return redirect()->back()->with('error', 'Gagal mengedit testimoni');
            }
        }
    }
}
